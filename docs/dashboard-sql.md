...
**Field name note, now resolved:** earlier versions of this doc referenced
...
**It's sent as a string, not a native number - cast it in SQL.** A prior
version sent this as a bare JSON `doubleValue`, which turned out to be
the actual cause of repeated `"invalid type: map, expected f64"`
ingestion failures - confirmed by inspecting the real failing payload
byte-for-byte, not guessed. It was the *only* unquoted JSON number
anywhere in the whole payload; every other numeric-looking attribute
(token counts, byte offsets) was already a quoted string and worked
fine. `llm.cost_usd` now matches that pattern, which means SQL needs
`CAST(llm_cost_usd AS DOUBLE)` wherever it's summed or sorted
numerically - already applied in every query below and throughout the
rest of this doc.
...
SELECT
  ...
  round(sum(CAST(llm_cost_usd AS DOUBLE)), 2) AS total_cost_usd,
...
SELECT
  ...
  round(sum(CAST(llm_cost_usd AS DOUBLE)), 2) AS cost_usd
FROM claude_code_history
WHERE event_type IN ('USER', 'ASSISTANT')
GROUP BY month
ORDER BY month
...
```sql
SELECT jira_ticket, repo_name, count(*) AS events, round(sum(cost), 2) AS cost_usd
FROM (
  SELECT repo_name, CAST(llm_cost_usd AS DOUBLE) AS cost,
...
panels. `Hrs Saved`/`Story Point Saved` are NOT - they depend on JIRA
data (story points, status-transition timestamps) that doesn't exist in
this stream at all, so no query can turn them into a live panel without
first ingesting JIRA's ticket lifecycle as its own stream (a separate,
non-trivial build - not something that exists yet).

**Getting bar/line charts to actually render:** pasting SQL alone isn't
enough for bar/line - OpenObserve needs fields explicitly assigned to
X-Axis and Y-Axis in the panel's own "Fields" section, separate from the
query editor. Table panels don't need this, which is why table works
immediately while bar/line don't until this step is done. Drag the
categorical/time column into X-Axis and the numeric column(s) into
Y-Axis (for a time-based X-Axis like a `month` column, mark its type as
**time**, not category).

**1. Headline KPIs, org-wide** (single aggregate row - genuinely can't
become bar/line regardless of field mapping; table, or a single-value
panel type if your version has one, is the *correct* choice here, not a
workaround):

```sql
SELECT
  sum(CASE WHEN event_is_prompt = true THEN 1 ELSE 0 END) AS total_prompts,
  sum(llm_usage_input_tokens + llm_usage_output_tokens + llm_usage_cache_read_input_tokens + llm_usage_cache_creation_input_tokens) AS total_tokens,
  round(sum(CAST(llm_cost_usd AS DOUBLE)), 2) AS total_cost_usd
FROM claude_code_history
WHERE event_type IN ('USER', 'ASSISTANT')
```

**2. Top tickets by cost** (bar chart - X-Axis: `jira_ticket`, Y-Axis:
`total_cost_usd`, or add `total_tokens`/`total_hrs_actual_work` too):

```sql
SELECT jira_ticket,
       sum(prompts) AS prompts_asked,
       sum(input_tokens + output_tokens + cache_read_tokens + cache_creation_tokens) AS total_tokens,
       round(sum(duration_seconds) / 3600.0, 2) AS total_hrs_actual_work,
       round(sum(cost_usd), 2) AS total_cost_usd
FROM (
  SELECT session_id, repo_name,
    sum(CASE WHEN event_is_prompt = true THEN 1 ELSE 0 END) AS prompts,
...
**3. Cost trend, monthly** (line chart - X-Axis: `month` as **time**
type, Y-Axis: `cost_usd`):

```sql
SELECT date_trunc('month', to_timestamp_micros(_timestamp)) AS month,
       round(sum(CAST(llm_cost_usd AS DOUBLE)), 2) AS cost_usd
FROM claude_code_history
WHERE event_type IN ('USER', 'ASSISTANT')
GROUP BY month
...
**4. Top repos by usage** (bar chart - X-Axis: `repo_name`, Y-Axis:
`total_tokens` or `total_hrs`):

```sql
SELECT repo_name,
       sum(input_tokens + output_tokens + cache_read_tokens + cache_creation_tokens) AS total_tokens,
       round(sum(duration_seconds) / 3600.0, 2) AS total_hrs
FROM (
  SELECT session_id, repo_name,
    sum(llm_usage_input_tokens) AS input_tokens,
    sum(llm_usage_output_tokens) AS output_tokens,
...
**5. Single-ticket drill-down** (same shape as the script's per-ticket
row - table, or bar with X-Axis: `repo_name`; use a dashboard Variable
for `$ticket` if you want it interactively swappable, though the exact
variable-binding syntax should be checked against your OpenObserve
version's own dashboard-variable UI rather than assumed):

```sql
SELECT jira_ticket, repo_name,
       sum(prompts) AS prompts_asked,
       sum(input_tokens + output_tokens + cache_read_tokens + cache_creation_tokens) AS total_tokens,
       round(sum(duration_seconds) / 3600.0, 3) AS total_hrs_actual_work,
       round(sum(cost_usd), 2) AS total_cost_usd
FROM (
  SELECT session_id, repo_name,
    sum(CASE WHEN event_is_prompt = true THEN 1 ELSE 0 END) AS prompts,
...
WHERE jira_ticket = 'GGLOBDRA-1970'
GROUP BY jira_ticket, repo_name
```
...
- **`llm_cost_usd`** is new as of Phase 10. It didn't exist before —
  ...
  assistant turns); absent (not zero) on others. **Sent as a string, not
  a native number** - cast with `CAST(llm_cost_usd AS DOUBLE)` for any
  arithmetic. It was originally a bare `doubleValue`, which turned out to
  be the actual cause of repeated OTLP ingestion failures
  (`"invalid type: map, expected f64"`) - it was the only unquoted JSON
  number anywhere in the whole payload, confirmed by inspecting a real
  failing payload directly rather than guessed at.
...
```sql
SELECT histogram(_timestamp) AS time, sum(CAST(llm_cost_usd AS DOUBLE)) AS cost_usd
FROM claude_code_history
...
FROM claude_code_history
WHERE llm_cost_usd IS NOT NULL
ORDER BY CAST(llm_cost_usd AS DOUBLE) DESC
LIMIT 20
```
